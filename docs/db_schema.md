...
- Для UUID требуется расширение `uuid-ossp` (`CREATE EXTENSION IF NOT EXISTS "uuid-ossp";`)
- Нет отдельных таблиц для профилей, матчей и сообщений (на текущем этапе всё упрощено)

## Рекомендации по улучшению (опционально на следующих этапах)

- Разделить `users` и `profiles`
- Добавить таблицу `matches`
- Вынести интересы в отдельную many-to-many таблицу
- Добавить timestamps (`created_at`, `updated_at`) во все таблицы# Database Schema
